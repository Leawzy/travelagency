...
<section class="container xl:mx-auto pt-5">
    ...
    $check_emp = mysqli_query($db, "SELECT * FROM `post`");
    $employees = mysqli_fetch_all($check_emp);
    ?>
    <h3 class="text-black text-3xl font-medium mb-8">Должности</h3>
    <table class="">
        <tr>
            <th class="border border-black">ID_Post</th>
            <th class="border border-black">Post Name</th>
            <th class="border border-black">Salary</th>
            <th class="border border-black">Responsibilities</th>
            <th class="border border-black">Requirements</th>
        </tr>
        <?php

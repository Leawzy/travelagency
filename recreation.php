...
<section class="container xl:mx-auto pt-5">
    ...
    $check_emp = mysqli_query($db, "SELECT * FROM `recreation`");
    $employees = mysqli_fetch_all($check_emp);
    ?>
    <h3 class="text-black text-3xl font-medium mb-8">Виды отдыха</h3>
    <table class="">
        <tr>
            <th class="border border-black">ID_Recreation</th>
            <th class="border border-black">Recreation Name</th>
            <th class="border border-black">Description</th>
            <th class="border border-black">Restrictions</th>
        </tr>
        <?php
